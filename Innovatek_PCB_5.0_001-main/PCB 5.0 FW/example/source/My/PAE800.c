	...
	{	PAEcommandFlag = FALSE;
		...
		if(Mode == MODE_AUTO){
			...
			{	case PREPURIFY:	        EnablePAEoutput(PAE_PrepurifyStepVoltageSetpoint*10, OptimizedPAE_CurrentForPrepureStep); break;
				...
				case WASTE:         
                                case HF_WASTE:          EnablePAEoutput(PAE_WasteStepVoltageSetpoint*10, OptimizedPAE_CurrentForWasteStep);                                    
                                     break;
				//case HF_WASTE:      
...
{	
          ...
          {	PAE_I2C_Stop();
                  no_of_byte_to_read = 0;
                   PutStringUsb("\r\nPAE ERROR: Start Condition Failed (R0)");
          ...
          {	PAE_I2C_Send(device_address);				//DEVICE ADDRESS
                  ...
                  {	PAE_I2C_Stop();
                          no_of_byte_to_read = 0;
                          PutStringUsb("\r\nPAE ERROR: Device Address NACK (R1)");
                          //Code Required For Testing
                  ...
                  {	PAE_I2C_Send(mem_start_add);
                          ...
                          {	PAE_I2C_Stop();
                                  no_of_byte_to_read = 0;
                                  PutStringUsb("\r\nPAE ERROR: Memory Start Address NACK (R2)");
                                  //Code Required For Testing
                          ...
                          {	PAE_I2C_Stop();
                                  ...
                                  {	PAE_I2C_Stop();
                                          no_of_byte_to_read = 0;
                                          PutStringUsb("\r\nPAE ERROR: Repeated Start Failed (R3)");
                                          //Code Required For Testing
                                  ...
                                  {	PAE_I2C_Send(device_address|0X01);				//DEVICE ADDRESS WITH READ COMMAND
                                          ...
                                          {	PAE_I2C_Stop();
                                                  no_of_byte_to_read = 0;
                                                  PutStringUsb("\r\nPAE ERROR: Device Address NACK on Read Command (R4)");
                                                  // Code Required For Testing
          ...
          if(PAE_I2C_ErrorFlag == TRUE){
              ...
              //RESET_DEVICE;
                  ...
                  PutStringUsb("\r\nPAE ERROR: Communication Failure During Read (R5)");
                  #ifdef DEBUG_I2C_ERROR
                          PutStringUsb("\r\nPAE > COMM. READ ERROR");
                  #endif
          }
...
{	
	...
	{	PAE_I2C_Stop();
		no_of_byte_to_write = 0;
                PutStringUsb("\r\nPAE ERROR: Start Condition Failed (W0)");
                return;
		//Code Required For Testing
	...
	{	PAE_I2C_Send(device_address);			//DEVICE ADDRESS
		...
		{	PAE_I2C_Stop();
			no_of_byte_to_write = 0;
                        PutStringUsb("\r\nPAE ERROR: Device Address NACK (W1)");
                        return;
			//Code Required For Testing
		...
		{	PAE_I2C_Send(mem_start_add);
			...
			{	PAE_I2C_Stop();
				no_of_byte_to_write = 0;
                                PutStringUsb("\r\nPAE ERROR: Memory Start Address NACK (W3)");
                                return;
				//Code Required For Testing
			...
			{	while(no_of_byte_to_write)
				{	PAE_I2C_Send(*buff);
					...
					{	PAE_I2C_Stop();
						no_of_byte_to_write = 0;
                                                PutStringUsb("\r\nPAE ERROR: Data Byte Write Failed (W4)");
                                                return;
						//Code Required For Testing
