...
{	PageToBuffer(SERVER_PAGE,PAGE_TO_BUFF1);
	...
	*/
	...
	u.c[0] = PREPURIFY_TIME_DEF_VAL;//Prepurify Time
        ...
        u.c[0] = SHUNT_PRE_CHARGE_TIME_DEF_VAL;
        ReadWriteMemBuffer(MEM_WRITE,SHUNT_PRE_CHARGE_TIME_IN_SECa,u.c,SHUNT_PRE_CHARGE_TIME_IN_SECn);
        
        #if(PRODUCT_TYPE==PRODUCT_DIENTRY)
            u.c[0] = SHUNT_PRE_CHARGE_TIME_DEF_VAL;
            ReadWriteMemBuffer(MEM_WRITE,SHUNT_PRE_CHARGE_TIME_IN_SECa,u.c,SHUNT_PRE_CHARGE_TIME_IN_SECn);
        #else
        u.c[0] = SHUNT_POST_CHARGE_TIME_DEF_VAL;
        ReadWriteMemBuffer(MEM_WRITE,SHUNT_POST_CHARGE_TIME_IN_SECa,u.c,SHUNT_POST_CHARGE_TIME_IN_SECn);
        ...
        u.c[0] = DYNAMIC_CAPDI_MODE_DEF_VAL;//CAPDI Mode
                ReadWriteMemBuffer(MEM_WRITE,DYNAMIC_CAPDI_MODEa,u.c,DYNAMIC_CAPDI_MODEn);
		      
        u.i[0] = PRE_PURE_CURRENT_DEF_VAL;//Purify Time
		ReadWriteMemBuffer(MEM_WRITE,PRE_PURE_CURRENTa,u.c,PRE_PURE_CURRENTn);
		
        u.i[0] = PURE_CURRENT_DEF_VAL;//Waste time
		ReadWriteMemBuffer(MEM_WRITE,PURE_CURRENTa,u.c,PURE_CURRENTn);
                
        u.i[0] = WASTE_CURRENT_DEF_VAL;//Waste time
		ReadWriteMemBuffer(MEM_WRITE,WASTE_CURRENTa,u.c,WASTE_CURRENTn);
        
        u.i[0] = PUMP_MANUAL_SPEED_DEF_VAL;//Pump Speed
		ReadWriteMemBuffer(MEM_WRITE,PUMP_MANUAL_SPEEDa,u.c,PUMP_MANUAL_SPEEDn);
        u.c[0] = PUMP_MANUAL_MODE_DEF_VAL;//WPump Mode
		ReadWriteMemBuffer(MEM_WRITE,PUMP_MANUAL_MODEa,u.c,PUMP_MANUAL_MODEn);
...
{	//SerialNoCheck();//to update serial valid flag, if serial code is sent though SMS
	...
	DirectMemRead(SHUNT_PRE_CHARGE_TIME_IN_SECa,u.c,SHUNT_PRE_CHARGE_TIME_IN_SECn);
	ShuntPostWasteStepTimeInSec = u.c[0];
	DirectMemRead(SHUNT_POST_CHARGE_TIME_IN_SECa,u.c,SHUNT_POST_CHARGE_TIME_IN_SECn);
	...
	SwitchPAEvoltPure = u.c[0];
        ...
        DirectMemRead(DYNAMIC_CAPDI_MODEa,u.c,DYNAMIC_CAPDI_MODEn);
	DynamicCapDIMode = u.c[0];
        DirectMemRead(PRE_PURE_CURRENTa,u.c,PRE_PURE_CURRENTn);
	PrePureCurrentSetpoint = u.i[0];
	DirectMemRead(PURE_CURRENTa,u.c,PURE_CURRENTn);
	PureCurrentSetpoint = u.i[0];
        DirectMemRead(WASTE_CURRENTa,u.c,WASTE_CURRENTn);
	WasteCurrentSetpoint = u.i[0];
	
        DirectMemRead(PUMP_MANUAL_SPEEDa,u.c,PUMP_MANUAL_SPEEDn);
	PumpManualSpeed = u.i[0];//Pump speed set manually 
        
        DirectMemRead(PUMP_MANUAL_MODEa,u.c,PUMP_MANUAL_MODEn);
	PumpManualMode = u.c[0];//Pump speed set manually 
	/*******************************************************************************/
	/*******************************************************************************/
	/*******************************************************************************/
	/*******************************************************************************/
