...
//	InitPAE800();
	...
	Ppg_SetCycle(5,PwmCycle);
        ...
        PumpManualMode = PUMP_MANUAL_MODE_OFF; //Set Manual mode to off
		...
		{	BOOTLOADER_LED_OFF;
            ...
            //GetAtlasReplacementConductivityReading();
				...
				{ 
                                  if (PumpManualMode == 1 ) 
                                    {   
                                      Ppg_SetDuty(5,PumpManualSpeed*2);                                    
                                    } else      {
                                      Ppg_SetDuty(5,0); 
                                    }                                  
	...
	{       default:
		case IDLE:			//DigitalOutput &= ~(1<<OUTPUT_SHUNT);	//OUTPUT_SHUNT ON
						...
						//IT WILL STAY IN IDLE UNTILL N UNLESS ENTER KEY IS PRESSED FROM HMI
                            Ppg_SetDuty(5,0);
                            DigitalOutput = PrevDigitalOutput = 0xFFFF; //ALL OUTPUT OFF for manual control from server while PCB in IDLE step.
		...
		case PURIFY:
                            ...
                            if((ProcessStepRunSec >= PurifyTimeInSec) || (ProcessStepRunSec > 10 && CurrentPAEvolt > SwitchPAEvoltPure))         {	//if end of step status is also to be logged then activate below LogRecord function
                                //LogRecord();
        ...
        */
                case WASTE: DigitalOutput &= ~(1<<OUTPUT_MIV);	//OUTPUT_MIV ON
                            if(ProcessStepRunSec < 2)//turn polarity contactor relay only after one sec
                              break;
                            //DigitalOutput |= (1<<OUTPUT_HFIV);//OUTPUT_HFIV OFF
                            //DigitalOutput |= (1<<OUTPUT_POV);	//OUTPUT_POV OFF
                            DigitalOutput &= ~(1<<OUTPUT_WOV);	//OUTPUT_WOV ON
                            ...
                            #endif
                            //if((ProcessStepRunSec >= PurifyTimeInSec) || (ProcessStepRunSec > 5 && CurrentPAEvolt > SwitchPAEvoltPure))  
                            if(FirstWasteCycle == FALSE)
                            {   if((ProcessStepRunSec >= WasteTimeInSec) || 
                                   (ProcessStepRunSec >= PureStepRunSec * WasteTimeInSec/(PrepurifyTimeInSec + PurifyTimeInSec)))
                                {   PostWasteTask();
                                }
                                if((ProcessStepRunSec >= 10) && (CurrentPAEvolt > SwitchPAEvoltWaste))
                            ...
                            {   if ((ProcessStepRunSec >= WasteTimeInSec/2) || 
                                    ((ProcessStepRunSec >= 10) && (CurrentPAEvolt > SwitchPAEvoltWaste)))
                                {
                                    PostWasteTask();
                                }
                            } 
...
{	//if end of step status is also to be logged then activate below LogRecord function
    ...
    */    
        ...
        if (DynamicCapDIMode == DYNAMIC_CAPDI_MODE_ON)      {
          ...
          OptimizedPAE_CurrentForPureStep = OptimizedPAE_CurrentForPrepureStep = (int)(DynaProportionalFactor * ((float)((float)LastCyclePureAverageConductivity/100) - TargetOutletConductivity) + OptimizedPAE_CurrentForPureStep);
          ...
          */
          ...
          OptimizedPAE_CurrentForPrepureStep = PrePureCurrentSetpoint * 100;
          OptimizedPAE_CurrentForPureStep    = PureCurrentSetpoint * 100;
          OptimizedPAE_CurrentForWasteStep   = WasteCurrentSetpoint * 100;
        }
        #ifdef DEBUG_PAE
            #if (PRODUCT_TYPE == PRODUCT_DISERIES)
        ...
        {	OptimizedPAE_CurrentForPureStep = PURE_MAX_CURRENT;
                OptimizedPAE_CurrentForPrepureStep = PURE_MAX_CURRENT;
            #ifdef DEBUG_PAE
