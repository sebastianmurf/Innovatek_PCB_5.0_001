 ...
 ** @link CanGroup Base Timer Module description @endlink
 **
 ** History:
 **   - 2012-02-21  1.0  MWi  First version.
 **   - 2012-07-09  1.1  MWi  CAN Prescaler setting check for CAN_MAX_CLK added
 **   - 2012-08-31  1.2  MWi  CANPRE address now defined in can.h
 **
 ******************************************************************************/
...
#if (defined(L3_PERIPHERAL_CAN_ACTIVE))
...
static void Can_InitInterrupt(volatile stc_cann_t* pstcCan)
{
  if (pstcCan == (stc_cann_t*)(&CAN0))
  {
    ...
    NVIC_SetPriority(CAN0_IRQn, L3_IRQ_LEVEL_CAN0);   
  }
  else if (pstcCan == (stc_cann_t*)(&CAN1))
  {
    NVIC_ClearPendingIRQ(CAN1_IRQn);
    NVIC_EnableIRQ(CAN1_IRQn);
    NVIC_SetPriority(CAN1_IRQn, L3_IRQ_LEVEL_CAN1);      
  }
...
static void Can_DeInitInterrupt(volatile stc_cann_t* pstcCan)
{
  if (pstcCan == (stc_cann_t*)(&CAN0))
  {
    ...
    NVIC_SetPriority(CAN0_IRQn, L3_DEFAULT_INTERRUPT_LEVEL);   
  }
  else if (pstcCan == (stc_cann_t*)(&CAN1))
  {
    NVIC_ClearPendingIRQ(CAN1_IRQn);
    NVIC_DisableIRQ(CAN1_IRQn);
    NVIC_SetPriority(CAN1_IRQn, L3_DEFAULT_INTERRUPT_LEVEL);      
  }
 ...
 ** porvided in the given config sturcture.
 ...
 **   - pstcCan == NULL
 **   - pstcConfig == NULL
 **   - pstcCanInternData == NULL (invalid or disabled CAN unit (L3_PERIPHERAL_ENABLE_CAN))
...
{
    stc_can_intern_data_t*  pstcCanInternData;
    stc_can_btr_t           unBTR     = {0};
    stc_can_ifncmsk_t       unIF1CMSK = {0};
    stc_can_ctrlr_t         unCTRLR   = {0};
 ...
 **
 ** \retval Ok                     Message buffer has been succesfully initialised.
 ** \retval ErrorInvalidParameter  If one of the following conditions are met:
 **   - pstcCan == NULL
 **   - pstcCanInternData == NULL (invalid or disabled CAN unit (L3_PERIPHERAL_ENABLE_CAN))
 **   - pstcMsgId == NULL
...
#endif // #if (defined(L3_PERIPHERAL_CAN_ACTIVE))
